...
export const listArrayDaynamicModify = (data: any, listName: string, fieldName: string) => {
  return data?.length > 0
    ? data?.map((item: any) => {
      const newItem = {
        id: item?.id?.toString(),
...
export const listArrayDaynamicModifyCustomId = (data: any, listName: string, fieldName: string, idField: string) => {
  return data?.length > 0
    ? data?.map((item: any) => {
      const newItem = {
...
export const listArrayModifyAdvanceSearch = (data: any, displayName: string) => {
  return data?.length > 0
    ? data?.map((item: any) => {
      const newItem = {
        id: item?.id?.toString(),
...
export const useDebouncedChange = <T>(initialValue: T, delay: number = 300): [T, (newValue: T) => any] => {
