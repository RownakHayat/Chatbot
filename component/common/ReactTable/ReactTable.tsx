...
const ReactTable: FC<IDataTable> = ({
    ...
    return (
        <div className="overflow-auto z-1 rounded">
            {isLoading ? <TableSkeleton dataSource={20} columns={6} /> : <table className="w-full bg-white dark:bg-background rounded-lg" ref={refVal}>
                                ...
                                {headerGroup.headers.map((header, idx) => (
                                    ...
                                    >
                                        {header.isPlaceholder ? null : (
                                            <div
                                                className={`flex justify-center gap-2 items-center ${header.id === "status" && "justify-center"
                        ...
                        table?.getRowModel().rows?.map((row: any) => (
                            ...
                            >
                                {row.getVisibleCells().map((cell: any) => (
                                    <td
                                        key={cell.id}
                                        className={`p-3 text-gray-600 w-fit border-[#D6DDFF] border-b-[1px] border-r-[1px] dark:${row?.original?.status === 0
                                            ? "text-slate"
                                            : "text-white"
                                            }`}
                                    >
            ...
            {paginationOff === true && <Pagination pagination={dataSource?.pagination} />}
        </div>
    )
}
